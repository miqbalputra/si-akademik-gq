    ...
    @foreach($students as $enrollment)
        ...
        <div class="flex items-center p-3 border {{ $isDailyAbsent ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300' }} rounded-xl transition-colors cursor-pointer" onclick="var el=document.getElementById('student_{{ $enrollment->id }}'); if(el) el.click();">
            <div class="flex items-center h-5">
                ...
                @else
                    <input id="student_{{ $enrollment->id }}" type="checkbox" name="absences[{{ $enrollment->id }}]" value="{{ $existingStatus ?? 'skipped' }}" {{ $existingStatus ? 'checked' : '' }} class="h-4.5 w-4.5 text-emerald-600 rounded border-slate-300 focus:ring-emerald-500 cursor-pointer" onclick="event.stopPropagation()">
                @endif
            </div>
