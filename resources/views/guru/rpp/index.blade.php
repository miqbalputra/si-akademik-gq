<x-layouts.portal title="RPP Saya" portalLabel="Portal Guru" breadcrumb="RPP Saya">
    <header class="portal-page-header">
        <div>
            ...
            <p class="mt-2 text-sm font-medium text-slate-500">Cari, ekspor, dan lihat RPP Diniyyah berdasarkan penugasan Anda.</p>
        </div>
        @unless(config('rpp_sync.enabled'))<a href="{{ route('guru.rpp.create') }}" class="btn btn-primary">Buat RPP</a>@endunless
    </header>
    ...
    <form class="ui-card mb-6 flex gap-3 rounded-2xl p-4" method="GET">
        ...
        @unless(config('rpp_sync.enabled'))<a class="btn btn-outline" href="{{ route('guru.rpp.trash') }}">Sampah</a>@endunless
    </form>
    ...
    <section class="ui-card overflow-hidden rounded-2xl">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[760px] text-left text-sm">
                ...
                @empty
                    <tr><td colspan="5" class="p-10 text-center text-slate-500">Belum ada RPP yang tersinkron dari Project RPP.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    @unless(config('rpp_sync.enabled'))<section class="ui-card mt-6 rounded-2xl p-5"><h2 class="font-black text-slate-800">Butuh bantuan RPP?</h2><p class="mt-1 text-sm text-slate-500">Kirim pesan ke Admin dan Kabag Diniyyah melalui pusat notifikasi.</p><form class="mt-3 flex flex-col gap-3 sm:flex-row" method="POST" action="{{ route('guru.rpp.help') }}">@csrf <input class="form-input flex-1" name="message" maxlength="2000" placeholder="Tulis kendala Anda" required><button class="btn btn-outline" type="submit">Kirim bantuan</button></form></section>@endunless
</x-layouts.portal>
