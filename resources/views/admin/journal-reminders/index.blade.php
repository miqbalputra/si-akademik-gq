    ...
    <section class="space-y-6">
        ...
        <section class="card-lg overflow-hidden">
            <div class="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                ...
                <div class="grid grid-cols-2 gap-2"><a href="{{ route('admin.journal-reminders.export', ['format' => 'png'] + $query) }}" class="btn min-h-11 border border-emerald-200 bg-emerald-50 text-emerald-800 hover:bg-emerald-600 hover:text-white">Unduh PNG Semua Guru</a><a href="{{ route('admin.journal-reminders.export', ['format' => 'pdf'] + $query) }}" class="btn min-h-11 border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-600 hover:text-white">Unduh PDF</a></div>
            </div>

            @forelse ($report['teachers'] as $teacher)
                <article class="border-b border-slate-100 p-5 last:border-b-0 sm:px-6">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 class="text-lg font-black text-slate-900">{{ $teacher['teacher_name'] }}</h3><p class="text-sm font-semibold text-slate-500">NIY: {{ $teacher['niy'] ?: '-' }}</p></div><div class="flex flex-wrap items-center gap-2"><span class="badge bg-rose-100 text-rose-800">{{ $teacher['missing_count'] }} jurnal kosong</span><a href="{{ route('admin.journal-reminders.export', ['format' => 'jpg', 'teacher_id' => $teacher['teacher_id']] + $query) }}" class="btn btn-sm border border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-600 hover:text-white">Unduh JPG</a><a href="{{ route('admin.journal-reminders.export', ['format' => 'png', 'teacher_id' => $teacher['teacher_id']] + $query) }}" class="btn btn-sm border border-emerald-200 bg-emerald-50 text-emerald-800 hover:bg-emerald-600 hover:text-white">Unduh PNG</a></div></div>
